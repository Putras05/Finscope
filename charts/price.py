...
def chart_price_history_plotly(res: dict, ticker: str,
    ...
    if split_date_str is not None:
        ...
        fig.add_annotation(
            ...
            y=0.96, yref='paper',  # v39: 1.02→0.96 đưa annotation VÀO TRONG chart, tránh đè title
            text=t('chart.train_test_split'),
            showarrow=False, xanchor='center',
            font=dict(size=9, color=T['text_secondary']),  # v39: 10→9 nhỏ gọn
    ...
    lay['title'] = dict(
        text=t('chart.price_history_title', ticker=ticker)
             + f'<span style="color:{T["text_muted"]};font-size:10px"> ({d0} → {d1})</span>',
        x=0.5, xanchor='center',
        font=dict(size=12, family='Inter', color=T['text_primary']),  # v38: 13→12 mobile
        pad=dict(t=4, b=4),
    )
    # v39: Override legend từ base.py (y=1.04) → đưa xuống dưới chart (y=-0.18)
    # để KHÔNG đè title + date range. Rangeselector buttons cũng ở y=-0.18 →
    # legend đặt y=-0.32 để không chồng rangeselector.
    lay['legend'] = dict(
        orientation='h',
        yanchor='top', y=-0.32,
    ...
    # v39: Tăng bottom margin để chứa rangeselector (y=-0.18) + legend (y=-0.32)
    lay['margin'] = dict(l=42, r=12, t=45, b=85)
