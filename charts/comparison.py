...
def chart_test_result_plotly(res: dict, ticker: str, method: str,
    ...
    _mup     = str(method).upper()
    _is_arima = 'ARIMA' in _mup
    _is_cart = 'CART' in _mup or 'TREE' in _mup
    _is_mlr  = 'MLR' in _mup
    _is_ar1  = ('AR' in _mup) and not _is_mlr and not _is_arima

    # ── Annotation text ──────────────────────────────────────────────────────
    if _is_arima:
        _order = res.get('order', None)
        _order_s = (f'ARIMA{tuple(_order)}' if _order else 'ARIMA')
        _aic = res.get('aic', float('nan'))
        _annot_text = (f'<b>{_order_s}</b><br>'
                       f'<b>R²</b> = {rv**2:.4f}  |  MAPE = {m_te["MAPE"]:.2f}%<br>'
                       f'AIC = {_aic:.1f}')
    elif _is_cart:
    ...
    if not show_scatter:
        ...
        fig.update_layout(
            ...
            height=440,
            margin=dict(l=42, r=15, t=50, b=55),  # v37: mobile-friendly
    ...
    fig.update_layout(
        height=560,
        margin=dict(l=42, r=15, t=55, b=80),  # v37: mobile-friendly
        paper_bgcolor=T['bg_card'],
        plot_bgcolor=T['bg_card'],
        font=dict(family='Inter, system-ui, sans-serif', size=11, color=T['text_primary']),
        hovermode='x unified',
...
# - T: hash theo is_dark (bool, ổn định qua rerun) thay vì id() — id(T) đổi
#   nếu theme() trả dict mới mỗi rerun (cache miss → rebuild 50-100ms).
# - ticker/interval/show_sma/show_ichimoku: primitives → Streamlit auto-hash.
# Toggle SMA / Ichimoku / Timeframe lần thứ 2 → cache hit thật → instant.
@st.cache_data(
    show_spinner=False, max_entries=64, ttl=600,
    hash_funcs={
...
def chart_price_candlestick(df: pd.DataFrame, ticker: str, T: dict,
    ...
    fig.update_layout(
        height=700,
        margin=dict(l=30, r=15, t=50, b=30),  # v37: r=70→15 mobile-friendly
